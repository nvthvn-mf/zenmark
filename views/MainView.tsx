...
const MainView: React.FC<MainViewProps> = ({user}) => {
    ...
    return (
        <div className="flex h-screen w-screen overflow-hidden bg-slate-50 font-sans">
            ...
            {/* --- SIDEBAR --- */}
            <aside
                className="w-20 lg:w-64 flex-shrink-0 border-r border-slate-200 flex flex-col bg-white transition-all duration-300">

                {/* Logo Area */}
                <div
                    className="h-16 flex items-center justify-center lg:justify-start lg:px-6 border-b border-slate-100">
                    <div
                        className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold shadow-md shadow-indigo-200">
                        Z
                    </div>
                ...
                <div className="mt-4 px-3 lg:px-6">
                    <div
                        className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 hidden lg:block">Documents
                        Récents
                    </div>
                        ...
                        {filteredDocs.slice(0, 10).map(doc => (
                            ...
                            >
                                <Icon name="file" size={16}
                                      className={`flex-shrink-0 ${activeDoc?.id === doc.id ? 'text-indigo-500' : 'text-slate-400'}`}/>
                                <span
                                    className="hidden lg:block truncate text-sm flex-1">{doc.title || 'Sans titre'}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="mt-auto p-4 border-t border-slate-100">
                    <div className="flex items-center justify-center lg:justify-between">
                        <div className="flex items-center gap-2">
                            <div
                                className={`w-2 h-2 rounded-full ${syncStatus === AppStatus.ONLINE ? 'bg-green-500' : 'bg-amber-500'}`}/>
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-slate-50 relative">
                ...
                {showProfile ? (
                    ...
                    <>
                        <header
                            className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-20">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    onClick={() => ExportController.exportToHTML(activeDoc.title, activeDoc.content)}
                                    className="p-2 text-slate-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 transition-all">
                                    <Icon name="download"/>
                                </button>
                                <button onClick={() => ExportController.exportToPDF()}
                                        className="p-2 text-slate-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 transition-all">
                                    <Icon name="search"/>
                                </button>
                            </div>
                ...
                <div
                    ...
                    <div
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3">
                            {versions.map(v => (
                                <div key={v.id}
                                     className="p-4 rounded-xl border border-slate-100 flex items-center justify-between hover:border-indigo-200 cursor-pointer"
                                     onClick={() => handleRollback(v)}>
                                    <div>
                                        <span className="font-bold text-slate-800">v{v.versionNumber}</span>
                                        <span
                                            className="text-xs text-slate-400 ml-2">{new Date(v.createdAt).toLocaleString()}</span>
                                    </div>
                                    <span
                                        className="text-xs text-indigo-600 font-medium bg-indigo-50 px-2 py-1 rounded">Restaurer</span>
                                </div>
                            ))}
                        </div>
                    </div>
